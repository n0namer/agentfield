...
## Next move
Do not reconcile the fork, publish product code, merge, or redeploy yet. BMAD `bmad-help` was activated from canonical `n0namer/BMAD-MNNZ`, classified the project as Implementation/Verification, and selected `bmad-qa-generate-e2e-tests` for the active gate. The immediate obligatory move is NOT another E2E run: current runtime source `c0923acdfca043c2c07e3d34daaa09e2a7e41d38` does not contain the verified contract-completion slice. First close the direct-target capability gap so `/core-src/agentfield-runtime` can be stale-safe patched in the existing `runtime-capture` container; then transplant only the already-verified `schema.go`/`runner.go`/`parity_test.go` delta, run targeted + full Go regression on that exact source, and only then execute Batch C E2E followed by Batch D golden regression. Current typed target-registry upsert is unavailable (`unregistered_operation_route`) and generic approval also has no executable adapter (`approval_capability_gap`); do not bypass these controls with opaque shell writes. Fork reconciliation becomes eligible only after runtime-loaded regression + E2E + golden DoD are PASS.
