...
## Next move
BMAD System-Level Test Design is now the governing acceptance workflow for SWE. Batch G1 inventory/traceability is CLOSED on CURRENT runtime evidence: `swe-planner` is active/ready at health 100 with 31 registered reasoners; deterministic component/package gates for discovery, config/runtime routing, DAG/replan primitives, issue/recovery/scope, coding/review, planning, advisor/verifier, gitops, CI and prompts are GREEN.

Current source state is intentionally not mislabeled canonical-clean: SWE remains `HEAD=.source-commit=0660fb8921f63b0a6ba58254124774068b92fe10` with the live `executor_test.go` contract fix dirty; AgentField remains `HEAD=.source-commit=26841718c42d2a7fb9008014f4a1d552f945bac6` with current live cancellation/structured-output/E2E hardening deltas. Those deltas have direct runtime/test evidence but still require normal SourceLoop canonicalization before a source-generation DONE claim.

**Next mandatory 30-minute Batch G2:** run the compact P0 technical E2E pack on the existing container stack, reusing fixtures rather than creating duplicate harnesses. It must cover actual discovery/routing, coder→reviewer→verifier lifecycle, strict scope/hygiene, structured-output completion, resume/recovery, root→child cancellation and zero-orphan termination. Gate: P0 TECH=100%, duplicate mutation=0, orphan process/child=0, silent strict-scope widening=0. Only after G2 GREEN proceed to P1 orchestration (advisor/replan/integration/parallel/failure-threshold) and then quality A/B. No PR/deploy/redeploy/release CI and no Coding Station.
