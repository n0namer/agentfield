...
- Verification is currently gated only by toolchain access on the direct runtime path: `runtime-capture` has no `/usr/local/go/bin/go`. The existing `agentfield-control-plane` registry entry is stale (`image_prefix=agentfield/control-plane`) while the actual current container is `debian:bookworm-slim`; this prevents direct scope binding for validation. Operator owner env has been updated to allow a bounded target-registry repair for `agentfield-control-plane`, and a single Coolify deploy is in progress to materialize that allowlist. No AgentField redeploy has been performed.
