...
**Next mandatory 30-minute Batch G4:** start the quality A/B pack on the same existing container stack and fixed `broker/fast-coding` baseline. Use the smallest high-information subset first: seeded reviewer fault, seeded verifier fault, explicit scope temptation, transient provider/recovery case, and one cross-component/integration case that actually requires `needs_integration_test=true`. Measure task success, fault-detection, scope precision, repair attempts, duplicate actions, trajectory violations, and terminal cleanup. Gate: raw quality dimensions are recorded with deterministic oracles; no single anecdotal run becomes a quality percentage; no PR/deploy/redeploy/release CI and no Coding Station.
