...
**Next mandatory 30-minute Batch G4:** source-level exact-spec/reviewer-spec-fidelity patch is affected/related GREEN and the only full-suite residual is differential-classified as clean-baseline disk-capacity debt. Now load the exact five-file delta into the existing same-container SWE runtime through its native package/reload path (no deploy/redeploy), then run one deterministic seeded-spec canary on fixed `broker/fast-coding`. If Coder preserves the explicit required literal and Reviewer blocks a deliberately drifted implementation, proceed to integration oracle v2 where each branch locally enforces exact `Register("shared")`, both branch-local suites PASS, merged repo deterministically FAILS, and Integration Tester must identify the merge-only fault. No Coding Station, PR, deploy/redeploy, or release CI. Measure task success, fault-detection, scope precision, repair attempts, duplicate actions, trajectory violations, and terminal cleanup. Gate: raw quality dimensions are recorded with deterministic oracles; no single anecdotal run becomes a quality percentage; no PR/deploy/redeploy/release CI and no Coding Station.
