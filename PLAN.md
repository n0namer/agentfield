...
**Next mandatory 30-minute Batch G4:** exact-spec runtime canary r5 (`exec_20260917_145342_bb4r4vpx`, run `run_20260917_145342_idhhhair`) proves the loaded container patch preserves the explicit description-only literal: Coder produced `RegistrationKey() == "shared"`, Reviewer returned `approved=true`, checkpoint `9847abe` changes only `registry.go`, and independent `go test ./...` PASS. The prior Verifier false-negative was traced to issue-level orchestration omitting `completed_issues[].files_changed`; `/src/swe-af/go/internal/issue/build.go` now forwards actual `filesChanged`, `build_test.go` adds `TestVerifierReceivesActualChangedFiles`, and full `internal/issue` PASS. After same-container rebuild/reload, Verifier prompt explicitly contained `Files changed: registry.go` and its schema-valid stable output is `passed=true` with evidence citing `return "shared"` plus passing tests, so the correctness defect is closed at functional-evidence level. The execution still exhibits the known post-schema-valid OpenCode lifecycle tail/recovery behavior, so root terminal PASS is not claimed. CURRENT next move: keep lifecycle hardening below correctness, use only public production-path entrypoints, and run integration oracle v2 where each branch locally enforces exact `Register("shared")`, both branch-local suites PASS, merged repo deterministically FAILS, and Integration Tester must identify the merge-only fault. INVALID if either branch alters the planted literal. No Coding Station, PR, deploy/redeploy, or release CI. Measure task success, fault-detection, scope precision, repair attempts, duplicate actions, trajectory violations, and terminal cleanup. Gate: raw quality dimensions are recorded with deterministic oracles; no single anecdotal run becomes a quality percentage.
