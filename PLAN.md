...
**Next mandatory 30-minute Batch G4:** exact-spec canary r5 remains functionally positive: Coder preserved description-only `RegistrationKey() == "shared"`, Reviewer approved exact implementation, checkpoint `9847abe` changes only `registry.go`, independent `go test ./...` PASS, and after the `files_changed` wiring fix Verifier's stable schema-valid output became `passed=true` with evidence citing `registry.go` and passing tests. The root run was then cancelled as a whole tree only to stop the known post-schema-valid OpenCode lifecycle tail; no matching OpenCode child remained, so this is lifecycle debt rather than correctness evidence. Anti-drift recovery also found an already-existing stronger integration fixture r3 at `/tmp/swe-g4-integration-fault-r3`: both branches independently implemented exact `Register("shared")`, merged commit `7f5eead` contains both exact calls, and independent merged `go test ./...` deterministically FAILS with `panic: duplicate registration: shared`. CURRENT control-plane ties this to run `run_20260917_124614_eqqp56xm`: both Reviewers succeeded on the exact literals, but Integration Tester `exec_20260917_125050_3dpjkjt1` returned `passed=true` and even wrote a test that treated the duplicate-registration panic as expected behavior. Its result is internally inconsistent (`tests_run=3`, `tests_passed=2`, `tests_failed=0`). Classification: **P0 Integration Tester false-positive / oracle corruption (Goodhart)**; integration detection is FAIL, not PASS. Live container source has now received an anti-Goodhart Integration Tester prompt/golden patch: run the existing merged baseline before writing new tests; any merged baseline crash/failure is integration failure unless the authoritative PRD explicitly requires it; new tests may reproduce/explain but must not swallow/normalize the failure; `passed=true` requires merged baseline success; counters must be consistent. Deterministic Go result invariant + regression is still pending because the production VPS Terminal control channel became unavailable after the prompt patch; DEV fallback correctly refused writes outside its registered live-patch roots, so no scope bypass was used. CURRENT next move: as soon as the existing container terminal is callable again, add the deterministic IntegrationTestResult invariant (`tests_run == tests_passed + tests_failed`, and `passed=true` only with zero failed/all-run passed), regress r3-like impossible output, run targeted gitops tests, same-container rebuild/reload, then replay r3 oracle through the public execute path. No Coding Station, GitHub programming, PR, deploy/redeploy, or release CI. Gate: Integration Tester must report merged duplicate-registration failure with concrete evidence; r3-like expected-failure normalization or inconsistent counters is FAIL.
