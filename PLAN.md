...
- Direct target topology is now explicit: control-plane consumes the source volume read-only, while runtime-capture owns the same core source volume read-write. Coding Station is not part of the active route. Typed live-patch roots currently do not expose `/core-src`, so core-source mutation is blocked by target mediation until the existing AgentField target binding permits that path; do not bypass this guardrail with opaque shell writes.
- Capability-gap root cause is now exact: the DEV VPS Terminal gateway runs from persistent hotfix entrypoint `/var/lib/vps-terminal/gateway-hotfix/server-live.mjs`. Its active hotfix `operation-mediation.mjs` does not register `TARGET_REGISTRY_UPSERT`, while `/app/gateway/operation-mediation.mjs` in the image already does. Therefore `/v1/target-registry/action` read succeeds but upsert deterministically returns `unregistered_operation_route`. Attempts to mutate the active hotfix by opaque target exec were correctly blocked as `REVIEW_REQUIRED`; guardrails were not bypassed.
- The existing runtime registry itself is healthy/readable at SHA `9a69919c2f5512190ef80807cbf24c6afe3ef55ef2ae2b6b0423b100951ed215`; `agentfield-dev-runtime-capture` is rev1 with `/capture-state` only. The intended bounded registry change remains rev1→rev2 with `workspace_root=/core-src/agentfield-runtime` and live roots `[/capture-state, /core-src/agentfield-runtime]` only.
- Two durable issue-write routes for the operator owner were attempted and unavailable: GitHub user token returned 403 for issue creation; GitHub App route reported `GITHUB_APP_NOT_CONFIGURED`. Until operator write authority is restored, this PLAN entry is the canonical durable blocker record.
...
- With pinned Go 1.25.1 + Python 3.12.11, workspace-local temp/cache, and the local SDK target on `PYTHONPATH`, the native resilience harness verified agent connectivity on attempt 1 and printed `Passed: 27`, `Failed: 0`, `All 27 tests passed!` across real control plane + three Python agents + mock LLM. This assertion result is strong E2E evidence but was not accepted as command PASS because cleanup hung and no process exit code was available.
- The cleanup hang was reproduced and rooted in the test harness: `mock_llm_server.py` handles SIGTERM by calling `server.shutdown()` from the same main thread running `serve_forever()`, which deadlocks. `run_tests.sh` cleanup was therefore bounded to TERM → 1s → KILL/reap and removal of the workspace-local control-plane binary. A full helper stop removed the orphan process tree; helper start/recreate is currently in progress. Batch C remains ACTIVE until the same 27/27 run returns exit code 0 and the public-Runner contract-completion E2E passes.
...
## Next move
Do not reconcile the fork, publish product code, merge, or redeploy yet. BMAD `bmad-help` was activated from canonical `n0namer/BMAD-MNNZ`, classified the project as Implementation/Verification, and selected `bmad-qa-generate-e2e-tests` for the active gate. The immediate obligatory move is NOT another E2E run: current runtime source `c0923acdfca043c2c07e3d34daaa09e2a7e41d38` does not contain the verified contract-completion slice. First repair the DEV VPS Terminal active-hotfix mediation drift so `TARGET_REGISTRY_UPSERT` is recognized in `/var/lib/vps-terminal/gateway-hotfix/operation-mediation.mjs`; then perform the single rev1→rev2 runtime-registry update for `agentfield-dev-runtime-capture`, stale-safe patch `/core-src/agentfield-runtime`, transplant only the already-verified `schema.go`/`runner.go`/`parity_test.go` delta, run targeted + full Go regression on that exact source, and only then execute Batch C E2E followed by Batch D golden regression. Do not bypass mediation with opaque shell writes. Fork reconciliation becomes eligible only after runtime-loaded regression + E2E + golden DoD are PASS.
