...
**Next mandatory 30-minute Batch G4:** finish the current exact-spec/reviewer-spec-fidelity slice before adding more scenarios. First resolve the current host-disk ENOSPC enough to run related/full Go validation on the already-staged five-file container delta; then prove the loaded runtime with one deterministic seeded-spec canary on fixed `broker/fast-coding`. Only after that run integration oracle v2 where each branch locally enforces exact `Register("shared")`, both branch-local suites PASS, merged repo deterministically FAILS, and Integration Tester must identify the merge-only fault. No Coding Station, PR, deploy/redeploy, or release CI. Measure task success, fault-detection, scope precision, repair attempts, duplicate actions, trajectory violations, and terminal cleanup. Gate: raw quality dimensions are recorded with deterministic oracles; no single anecdotal run becomes a quality percentage; no PR/deploy/redeploy/release CI and no Coding Station.
