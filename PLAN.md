...
## Next move
Do not publish, merge, reconcile forks, deploy, redeploy, or run release CI. The exited-workforce lifecycle gap is RESOLVED container-first: DEV Terminal now resolves/restarts the same exited workforce container via an existing startup hotfix, with `node_check` PASS and no new AgentField container. The workforce reconcile guard was stale-safe patched so the pinned commit only needs to exist; accepted runtime state was reconciled successfully from `58c4e0d19081bc52363c120b7963a34cebb1e894` to `6f5b4382e6231721f60be7045b9d91fd85e34fb5` while preserving `PLAN.md` (`RECONCILED_ACCEPTED_CAPTURE ... plan_preserved=1`).

The next bootstrap blocker was a real AgentField installer bug: `af install /src/swe-af/go` inherited the caller `GOWORK` while invoking `go build -mod=mod`, which Go rejects in workspace mode. Runtime-authoritative source now contains a regression test plus a minimal installer fix in `control-plane/internal/packages/gointerp.go` that runs installed package builds with `GOWORK=off`. The already-existing `control-plane-build` container was restarted (same container ID, no new container), canonical regression/build completed Exit 0, and `/build/af` was rebuilt (SHA-256 `048c855d810258fe3bfc41430ca630888e27edb032714cdc017b6db05658404f`).

The same workforce then completed package installation and started `swe-planner` on port 8800. Runtime log proves `node.register.complete` and publishes the full native reasoner set including `implement_issue`; however the immediate status update and subsequent lease renewals fail with `409 stale_agent_instance`. Control-plane logs independently confirm the same `rejecting stale agent instance update` events. This is the current blocker. The immediate container-first move is to fix the registration/lease consistency contract so a newly registered same-node/same-version process becomes the current instance, then pass native discovery + one non-mutating reasoner smoke. Only after that green gate may `implement_issue` be invoked for the first bounded coding task, followed by canonical tests and the recovery canary.
