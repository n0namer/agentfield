...
- User explicitly authorized one ephemeral `control-plane-build/go-reconcile` builder on 2026-09-14. Execution-path discovery then proved the remaining blocker is capability, not authorization: `coolify-control` has no Docker socket; `vps-terminal-dev` exposes a guarded sockguard socket, but direct Docker API calls are blocked by operator mediation as `REVIEW_REQUIRED`, and the current typed tool surface has no create/start/remove arbitrary-container operation. No raw-socket bypass or persistent temporary service was created. Regression therefore remains NOT RUN on the runtime-authoritative dirty source despite authorization.
