...
- For advisor/replan/verifier/integration testing run paired OFF/ON comparisons where meaningful; critical nondeterministic scenarios get repeated runs only after deterministic TECH PASS. Next high-information scenario is a two-branch cross-component fixture that forces `needs_integration_test=true` so Integration Tester quality is measured on a real runtime path rather than inferred from component tests.
