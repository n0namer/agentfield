...
func loadConfig(configFile string) (*config.Config, error) {
	...
	// ExecutionCleanupConfig struct tags are documentation only; Viper leaves
	// omitted values at Go zero values unless defaults are registered explicitly.
	// Without these defaults the unified `af server` path silently disables the
	// stale-execution reaper. Explicit config values still retain precedence.
